...
intersect (HRay o d) (HSphere p r) = let
  omp = o - p :: HVec3
  ...
  in 
    if discriminant <= 0 then
      ...
      in 
      Just ((HSphere p r),min t1 t2)
        
    

intersect (HRay o d) (HPlane p n) = let dp = d `dot` n in
  if dp == 0 then
    Nothing
  else
...
surfaceNormal (HPlane _ n) _ = n
surfaceNormal (HSphere p n) i = norm (p --> i)
